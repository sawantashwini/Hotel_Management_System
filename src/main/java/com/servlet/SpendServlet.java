...
public class SpendServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SpendService ser = new SpendService();
		File savesFile = null;
		FileItem item = null;
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			while (itr.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if (name.equals("Cash_amount"))dto.setDebit_cash_amount(Float.parseFloat(item.getString() == null ? "0" : item.getString().trim()));
					
					if (name.equals("Online_amount"))dto.setDebit_online_amount(Float.parseFloat(item.getString() == null ? "0" : item.getString().trim()));
					...
					if (name.equals("Spend_name"))
						dto.setSpend_name(item.getString() == null ? "" : item
								.getString().trim());
					...
					//Float
					if (name.equals("Online_amount"))pay_dto.setDebit_online_amount(Float.parseFloat(item.getString() == null ? "0" : item.getString().trim()));
					
					if (name.equals("Cash_amount"))pay_dto.setDebit_cash_amount(Float.parseFloat(item.getString() == null ? "0" : item.getString().trim()));
					...
					if (name.equals("Remark"))
						pay_dto.setRemark(item.getString() == null ? "" : item
								.getString().trim());
					if (name.equals("Type"))pay_dto.setType("Spend");
				} else {
					...
					if (item.getSize() != 0) {
						if (item.getSize() < 3000000) {
							if (dto.getId() == 0) {
								...
								savesFile = new File(path + "SpendImage/" + j
										+ ".jpg");
								...
								savesFile = new File(path + "SpendImage/"
										+ dto.getId() + ".jpg");
								try {
									item.write(savesFile);
		...
		if (dto.getId() == 0) {
			...
			b = ser.insertSpendInfo(dto,pay_dto, request, config);

			if (b > 0) {
				response.sendRedirect("add_spend.jsp?msg=Yes");
			} else {
				response.sendRedirect("add_spend.jsp?msg=No");
			}
		}
		// condition for updating
			...
			c = ser.updateSpendInfo(dto,pay_dto, request, config);

			if (c) {
				response.sendRedirect("manage_spend.jsp?msg=YesUp");
			} else {
				response.sendRedirect("manage_spend.jsp?msg=NoUp");
			}
		}
