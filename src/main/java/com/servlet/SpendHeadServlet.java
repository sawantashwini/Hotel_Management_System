...
import com.service.SpendHeadService;

/**
 * Servlet implementation class SpendHeadServlet
 */
@WebServlet("/SpendHeadServlet")
public class SpendHeadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (dto.getId() == 0) {
			...
			b = ser.insertSpendHead(dto, request, config);

			if (b) {
				response.sendRedirect("add_spend_head.jsp?msg=Yes");
			}else {
				response.sendRedirect("add_spend_head.jsp?msg=No");
			}
		}
		// condition for updating
		else {

			b = ser.updateSpendHead(dto, request, config);

			if (b) {
				response.sendRedirect("manage_spend_head.jsp?msg=YesUp");
			}else {
				response.sendRedirect("manage_spend_head.jsp?msg=NoUp");
			}
		}
	}
