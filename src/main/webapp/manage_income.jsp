...
<body>
	...
	<main id="main" class="main">
		...
		<%@include file="include/add_pages.jsp"%>
			<h1 id="page_title">Income Report</h1>
		</div>
								...
								<div class="col-md-3 refresh-container refresh-block-none mb-3" data-list="list_spend_head">
									<div class="control form-floating refresh-input">
										<input type="text" class="form-control" id="name" name="Name"
											value="<%=name%>" placeholder="Name" autocomplete="off" required> 
						...
						<table id="example" class="table hover table-responsive nowrap">
							<thead class="text-center" id="thead">
								<tr>
									...
									<th style="width: 150px; text-align: center;">Income on</th>
							...
							<tbody id="tbody" class="text-center">
								...
								ArrayList<IncomeDto> list = service.getIncomeInfo(d1, d2, name, config, request);
		...
		: LogFileService.changeFormate(dto.getOnline_date(), "yyyy-MM-dd", "dd-MM-yyyy")%></td>
									...
									<td><a class="main-color"
										href="edit_income.jsp?id=<%=dto.getId()%>"><i
									...
									<td><a
										...
										href="AjaxSpendIncomeDelete.jsp?income_id=<%=dto.getId()%>">
	...
	function printfunction(){
		return '<h1 style="text-align:center;margin-bottom:10px;">'+page_title+'</h1><br><h5 style="text-align:center;margin-bottom:2px;">From Date: <%=first_date%> To Date: <%=second_date%> Name:<%=name%></h5>';
	}
	function printOtherfunction(){
		return 'From Date: <%=first_date%> To Date:<%=second_date%> Name:<%=name%>
		';
		}
		function printTitle() {
			return page_title;
